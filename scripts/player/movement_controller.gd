...
@onready var parcel: Parcel = $"../Parcel"
...
func animate_to_cell(new_cell: Vector2i) -> void:
...
func move(dir: Vector2i) -> void:
	...
	if grid.is_portal(next) and grid.get_portal(next)["dir"] == dir:
		...
		await animate_to_cell(player.current_cell)
...
func dash(dir: Vector2i) -> void:
	...
	var curve = get_blackhole_curve(dir, dash_length)
	...
	for cell in dash_path:
		...
		if grid.is_debris(cell) or grid.is_house(cell) or hazards.is_on_blackhole(cell):
			await animate_to_cell(cell if grid.is_in_bounds(cell) else dash_path[-2])
			await game.game_over()
			return

		if hazards.is_asteroid_at(cell) or hazards.is_pirate_at(cell):
			await animate_to_cell(cell)
			await game.game_over()
			return
	...
	if used_portal:
		...
		await animate_to_cell(stop_cell)

	var behind = start_cell - dir
	if not await destroy_behind(behind):
		return
	...
	if not await parcel.check_fragile():
		return
...
func get_blackhole_curve(dir: Vector2i, dash_length: int) -> Vector2i:
	for b in hazards.blackholes:
		for i in range(1, dash_length + 1):
			...
			if b.is_yellow_zone(path_cell) or b.is_red_zone(path_cell):
				return get_curve_direction(dir, b.cell)

	return Vector2i.ZERO


func destroy_behind(behind: Vector2i) -> bool:
	if not grid.is_in_bounds(behind):
		return true
